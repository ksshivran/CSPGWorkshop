...
void 
GoosePublisher_setPrivateKey(GoosePublisher self, char* privateKey)
{
   if (self->privateKey)
        GLOBAL_FREEMEM(self->privateKey);
...
GoosePublisher_publish(GoosePublisher self, LinkedList dataSet)
{
    ...
    /* APDU  DataFrame */
    uint8_t* apduStart = buffer;
    int32_t apduLen = 0;   
    
     /*self->payloadLength = createGoosePayload(self, dataSet, buffer, maxPayloadSize);*/
    apduLen = createGoosePayload(self,dataSet, buffer, maxPayloadSize);
    ...
    /* Add extension as HMAC */
    uint8_t* extStart = buffer+ apduLen;
    ...
    /* Copy HMAC to payload */
    memcpy(extStart,mac,macLen); 
    
    extLen  = (int32_t) macLen;
    
    /*Reserved1 =extension length */
    int r1_index = self->payloadStart - 4;
    self->buffer[r1_index]     = (uint8_t)(extLen >> 8) & 0xff;
    self->buffer[r1_index + 1] = (uint8_t)(extLen & 0xff);


    self->payloadLength = apduLen +extLen;   
