...
/* ON position → 10 */
    MmsValue_setBitStringBit(stVal, 0, false);   // MSB
    MmsValue_setBitStringBit(stVal, 1, true);
...
/* -------- XCBR.Pos.t (Timestamp UTC) -------- */
    ...
    if (publisher) {
        GoosePublisher_setPrivateKey(publisher, privateKey);
