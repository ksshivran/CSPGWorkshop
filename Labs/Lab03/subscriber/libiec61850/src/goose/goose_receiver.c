...
void
GooseReceiver_setPrivateKey(GooseReceiver self, const char* privateKey)
{
    if (self->privateKey != NULL)
        GLOBAL_FREEMEM(self->privateKey);

    self->privateKey = StringUtils_copyString(privateKey);
}
 
const char*
GooseReceiver_getPrivateKey(GooseReceiver self)
{
    if (self->privateKey)
...
parseGooseMessage(GooseReceiver self, uint8_t* buffer, int numbytes)
{
    ...
    /* Get HMAC lenght from reserved field 1 */
    int extLen = (buffer[bufPos++] << 8) | buffer[bufPos++];    
    /* Skip reserved field 2*/ 
    bufPos += 2;
    ...
    /* HMAC Verificaiton */
     if(extLen>0){
      ...
      uint8_t* apduStart = buffer + bufPos; 
      uint8_t* extStart = buffer+bufPos+ apduLength;
          
     /* HMAC Extension Size existence check in the Frame */
     /* uint8_t* frameEnd = buffer+numbytes; 
      if (extStart + extLen > frameEnd) {
        #if (DEBUG_GOOSE_SUBSCRIBER)
                printf("GOOSE_SUBSCRIBER: HMAC extension exceeds frame size -> drop\n");
        #endif
        return; 
      }*/
      
     const unsigned char *key = (const unsigned char *) self->privateKey;
     size_t keyLen = strlen((const char *) self->privateKey);


     unsigned char calcMac[EVP_MAX_MD_SIZE];
     unsigned int calcMacLen = 0;
     if (HMAC(EVP_sha256(),
          key, (int) keyLen,
          apduStart, (size_t) apduLength,
          calcMac, &calcMacLen) == NULL){ 
                printf("GOOSE_SUBSCRIBER: HMAC() failed -> drop\n");
          return; /* drop frame */
      }
     /* For SHA-256 we expect 32 bytes, but we just use extLen */
      if ((int) calcMacLen != extLen) {
        printf("GOOSE_SUBSCRIBER: HMAC length mismatch (got %u, expected %d) -> drop\n",
                       calcMacLen, extLen);
        return; /* drop frame */
      }
     /* Compare HMACs */
      ...
      for (int i = 0; i < extLen; i++) {
           diff |= (calcMac[i] ^ extStart[i]);
      }
      if (diff != 0){ 
         printf("GOOSE_SUBSCRIBER: HMAC mismatch -> drop frame\n");
        return; /* drop frame */
       }

        printf("GOOSE_SUBSCRIBER: HMAC verified OK\n");
       }
        else {
            /* extLen == 0: either accept or drop */
            printf("GOOSE_SUBSCRIBER: No HMAC extension present (extLen=0)\n");
        }    
